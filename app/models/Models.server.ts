...
export interface ExtendedQRCode extends QRCode {
    productTitle: string | '';
    productImage: string | '';
    productAlt: string | '';
...
export async function getQRCode({ id, graphql } : Pick<QRCode, "id"> & {graphql: any}) {
  ...
  export async function findQRCode({ id } : Pick<QRCode, "id"> ) {
  ...
  export function getDestinationUrl(qrCode: QRCode & { profile?: Profile }) {
  ...
  function extendQRCode(qrCode: QRCode, product: any) {
    return {
      ...qrCode,
